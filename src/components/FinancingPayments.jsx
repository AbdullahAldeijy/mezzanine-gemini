...
import {
  ...
  CreditCard, Clock, ArrowDownCircle, ArrowUpCircle,
} from 'lucide-react';
...
export const FinancingPaymentsContent = ({ payments, onPay, disb, onToggleDisb, footer }) => {
  const [activeSection, setActiveSection] = useState('disbursements');
  ...
  return (
    <div>
      ...
      {/* ── Section tabs — same style as Role Groups / Team Members ── */}
      <div className="flex gap-2 mb-6 bg-white/70 backdrop-blur-md rounded-xl p-1 shadow-sm">
        ...
        >
          ...
          Fund Disbursements
          <span className="text-[10px] opacity-70">صرف التمويل</span>
        </button>
        <button
        ...
        >
          ...
          Mezzanine Finance Payments
          <span className="text-[10px] opacity-70">مدفوعاتي</span>
        </button>
      </div>

      {/* ── Section 1: Fund Disbursements ──────────────────────────── */}
      {activeSection === 'disbursements' && (
        <div>
          <div className="flex items-baseline justify-between mb-4 md:mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900">Fund Disbursements</h2>
          ...
          <div className="bg-white/90 backdrop-blur rounded-2xl shadow-sm p-4 md:p-6 mb-6">
      ...
      {/* ── Section 2: Mezzanine Finance Payments — exact CRM style ── */}
      {activeSection === 'repayments' && (
        <div>
          <div className="flex items-baseline justify-between mb-4 md:mb-6">
          ...
          {/* Teal gradient card — exact CRM style */}
          <div className="bg-gradient-to-br from-teal-500 to-teal-700 rounded-2xl shadow-sm p-4 sm:p-6 md:p-8 text-white mb-6 md:mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs sm:text-sm font-semibold leading-tight">Mezzanine Finance Facility — 6 Monthly Installments</span>
            </div>
            <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4">
              <div>
                <p className="text-[10px] sm:text-xs text-white/70 mb-0.5">Total Financed</p>
                <p className="text-sm sm:text-xl md:text-2xl font-bold leading-tight tabular-nums">{fmt(total)}<span className="block text-[9px] sm:text-xs text-white/60">SAR</span></p>
              </div>
              <div>
                <p className="text-[10px] sm:text-xs text-white/70 mb-0.5">Paid So Far</p>
                <p className="text-sm sm:text-xl md:text-2xl font-bold leading-tight tabular-nums">{fmt(paid)}<span className="block text-[9px] sm:text-xs text-white/60">SAR</span></p>
              </div>
              <div>
                <p className="text-[10px] sm:text-xs text-white/70 mb-0.5">Remaining</p>
                <p className="text-sm sm:text-xl md:text-2xl font-bold leading-tight tabular-nums">{fmt(remaining)}<span className="block text-[9px] sm:text-xs text-white/60">SAR</span></p>
              </div>
            </div>
          ...
          <div className="bg-white/90 backdrop-blur rounded-2xl shadow-sm p-4 md:p-6 mb-6 md:mb-8">
            ...
            <div className="space-y-3">
              {payments.map((p) => (
                <div key={p.id} className="flex items-center justify-between gap-2 p-3 md:p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs sm:text-sm font-bold text-slate-900 tabular-nums">{fmt(p.amount)} SAR</span>
                    {p.status === 'Paid' ? (
